...
import { Search, Filter, Plus, User, Mail, Phone, MoreHorizontal, Building2, Loader2, UserCircle2 } from 'lucide-react';
    ...
    <div className="space-y-12 pb-20 bg-[#050505] min-h-screen -m-6 lg:-m-16 p-6 lg:p-16 text-white font-body">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
         <Card className="bg-white/[0.02] border-white/5 rounded-none p-8 space-y-4">
            ...
            <p className="text-3xl font-headline">{contacts.length}</p>
         </Card>
         <Card className="bg-white/[0.02] border-white/5 rounded-none p-8 space-y-4">
            <p className="text-[9px] font-bold text-slate-500 uppercase tracking-widest">Strategic Partners</p>
            <p className="text-3xl font-headline">{contacts.filter(c => (c as any).tier === 'Strategic').length || 0}</p>
         </Card>
         <Card className="bg-white/[0.02] border-white/5 rounded-none p-8 space-y-4">
      ...
      <Card className="bg-white/[0.02] border-white/5 rounded-none overflow-hidden">
        ...
        {loading ? (
          <div className="py-40 flex flex-col items-center justify-center space-y-6">
             ...
             <p className="text-[10px] uppercase tracking-[0.5em] text-slate-700 font-bold">Synchronizing Encrypted Vault...</p>
          </div>
        ) : contacts.length === 0 ? (
          <div className="py-40 flex flex-col items-center justify-center space-y-6">
             <UserCircle2 className="text-slate-800" size={48} />
             <p className="text-[10px] uppercase tracking-[0.5em] text-slate-700 font-bold">No Records Found in Global Hub</p>
          </div>
        ) : (
          <Table>
            <TableHeader className="bg-white/[0.03]">
              <TableRow className="border-white/5 hover:bg-transparent">
                ...
                <TableHead className="text-[9px] font-bold uppercase tracking-[0.2em] text-slate-500 py-6">Protocol / Scope</TableHead>
                <TableHead className="text-[9px] font-bold uppercase tracking-[0.2em] text-slate-500 py-6 font-center text-right pr-10">Status</TableHead>
              </TableRow>
            </TableHeader>
            <TableBody>
              {contacts.map((c) => (
                <TableRow key={c.id} className="border-white/[0.02] hover:bg-white/[0.01] transition-colors">
                  <TableCell className="py-8 px-10 font-bold text-[10px] text-slate-600 truncate max-w-[80px]">#{c.id.slice(0, 5)}</TableCell>
                  <TableCell className="py-8">
                    <div className="flex items-center space-x-4">
                        ...
                        <div className="space-y-1">
                          ...
                          <p className="text-[9px] text-slate-500 font-bold tracking-widest uppercase">{c.email || 'N/A'}</p>
                        </div>
                    </div>
                  </TableCell>
                  <TableCell className="py-8">
                    <div className="flex items-center space-x-2 text-[10px] font-bold uppercase tracking-widest text-slate-400">
                        ...
                        <span>{(c as any).organizationName || 'Strategic Individual'}</span>
                    </div>
                  </TableCell>
                  <TableCell className="py-8">
